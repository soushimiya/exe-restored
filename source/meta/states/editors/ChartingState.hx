...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		bpmTxt = new FlxText(1040, 50, 0, "", 16);
		...
		textBox.setPosition(((UI_box.width - textBox.width) / 2) + UI_box.x, (UI_box.height + UI_box.y) + 10);
	...
	function addSongUI():Void
	{
		...
		var stepperKeys:FlxUINumericStepper = new FlxUINumericStepper(stepperBPM.x + (stepperBPM.width * 2), 70, 3, 4, 4, 7);
		...
		tab_group_song.add(new FlxText(stepperBPM.x + 100, stepperBPM.y - 15, 0, 'Key count:'));
	...
	function addVisualsUI():Void
	{
		...
		var gradient1colors = new FlxUIInputText(10, 30, 150, '${ClientPrefs.editorGradColors[0].red}, ${ClientPrefs.editorGradColors[0].green}, ${ClientPrefs.editorGradColors[0].blue}', 8);
		var gradient2colors = new FlxUIInputText(10, 50, 150, '${ClientPrefs.editorGradColors[1].red}, ${ClientPrefs.editorGradColors[1].green}, ${ClientPrefs.editorGradColors[1].blue}', 8);
		...
		var changecolors:FlxButton = new FlxButton(180, 37.5, "Change colors", function()
		{
			...
			remove(gradient);
			gradient = FlxGradient.createGradientFlxSprite(FlxG.width, FlxG.height * 8, [0x0, FlxColor.fromRGB(grad1Colors[0], grad1Colors[1], grad1Colors[2]), FlxColor.fromRGB(grad2Colors[0], grad2Colors[1], grad2Colors[2]), 0x0]);
		...
		var boxTxtColors1 = new FlxUIInputText(10, 115, 150, '${ClientPrefs.editorBoxColors[0].red}, ${ClientPrefs.editorBoxColors[0].green}, ${ClientPrefs.editorBoxColors[0].blue}', 8);
		var boxTxtColors2 = new FlxUIInputText(10, 135, 150, '${ClientPrefs.editorBoxColors[1].red}, ${ClientPrefs.editorBoxColors[1].green}, ${ClientPrefs.editorBoxColors[1].blue}', 8);
		...
		var uiBoxTxt = new FlxUIInputText(10, 175, 150, '${ClientPrefs.editorUIColor.red}, ${ClientPrefs.editorUIColor.green}, ${ClientPrefs.editorUIColor.blue}', 8);
		...
		var newPrsName = new FlxUIInputText(10, 240, 150, '', 8);

		var lPrs = new FlxText(10, 260, 0, "Load Preset", 6);
		var prsList = new FlxUIDropDownMenuCustom(10, 270, FlxUIDropDownMenuCustom.makeStrIdLabelArray(ClientPrefs.chartPresetList), function(preset:String){
		...
		var newPrsButton = new FlxButton((newPrsName.x + newPrsName.width) + 10, 240, "New Preset", function(){
			if(!ClientPrefs.chartPresets.exists(newPrsName.text))
				ClientPrefs.chartPresetList.push(newPrsName.text);
			ClientPrefs.chartPresets.set(newPrsName.text, [[ClientPrefs.editorGradColors[0], ClientPrefs.editorGradColors[1]], false, [ClientPrefs.editorBoxColors[0], ClientPrefs.editorBoxColors[1]], ClientPrefs.editorUIColor]);
			ClientPrefs.saveSettings();

			prsList.setData(FlxUIDropDownMenuCustom.makeStrIdLabelArray(ClientPrefs.chartPresetList));
		...
		var clearPresets = new FlxButton((prsList.x + prsList.width) + 10, 270, "Clear Presets", function(){
			openSubState(new Prompt('This action will clear all presets.\n\nProceed?', 0, function(){
				...
				ClientPrefs.chartPresets.set('Default', [[FlxColor.fromRGB(0,0,0), FlxColor.fromRGB(0,0,0)], false, [FlxColor.fromRGB(255,255,255), FlxColor.fromRGB(210, 210, 210)], FlxColor.fromRGB(250,250,250)]);
				ClientPrefs.saveSettings();
	...
	function addSectionUI():Void
	{
		...
		var mirrorButton:FlxButton = new FlxButton(duetButton.x + 100, duetButton.y, "Mirror Notes", function()
		{
			...
			for (note in _song.notes[curSec].sectionNotes)
			{
				...
				note[1] = boob;
				var copiedNote:Array<Dynamic> = [note[0], boob, note[2], note[3]];
				if(_song.keys == 7){ for( i in 4...7){ copiedNote.push(note[i]); }}
				//duetNotes.push(copiedNote);
			}
	...
	override function update(elapsed:Float)
	{
		...
		gradient.y = FlxMath.lerp(gradient.y, gradient.y - 10, CoolUtil.boundTo(elapsed * 3, 0, 1));
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				MusicBeatState.switchState(new meta.states.editors.MasterEditorMenu());
			...
			if(FlxG.keys.justPressed.ESCAPE && FlxG.keys.pressed.SHIFT) {	
				if(startTime == 0)
					playSongFromTimestamp(FlxG.sound.music.time);
				else
					playSongFromTimestamp(startTime);
				...
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					if (curSelectedNote != null)
					{
						...
						if(controlArray.contains(true))
						{
							...
							{
								if(controlArray[i])
									if(curSelectedNote[1] == i) curSelectedNote[2] += datime - curSelectedNote[2] - Conductor.stepCrotchet;
							}
							updateGrid();
							updateNoteUI();
						}
		...
		if(FlxG.mouse.overlaps(clickForInfo)){
			...
			if(FlxG.mouse.justPressed){
				...
				FlxTween.color(textBox, 0.75, FlxColor.BLACK, FlxColor.fromRGB(ClientPrefs.editorUIColor.red, ClientPrefs.editorUIColor.green, ClientPrefs.editorUIColor.blue), {ease: FlxEase.quartOut});
				FlxTween.tween(textBox, {x: 0, y: 0, alpha: 1}, 0.75, {ease: FlxEase.quartOut});
				FlxTween.tween(textBox.scale, {x: 8.25806451613, y: 34.2857142857}, 0.75, {ease: FlxEase.quartOut, onComplete: function(shit:FlxTween){
	...
	*/
	...
	function setUIStuffPos(){
		if(_song.keys == 4){
			bpmTxt.setPosition(1040, 50);
			UI_box.setPosition(640 + GRID_SIZE / 2, 25);	
		}else{
			bpmTxt.setPosition(1040, 500);
			UI_box.setPosition(840 + GRID_SIZE / 2, 25);
		}
	}
	...
	function reloadGridLayer() {
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * ((_song.keys * 2) + 1), Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]), true, ClientPrefs.editorBoxColors[0], ClientPrefs.editorBoxColors[1]);
		#if desktop
		...
		{
			prevGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * ((_song.keys * 2) + 1), Std.int(GRID_SIZE * getSectionBeats(curSec - 1) * 4 * zoomList[curZoom]), true, FlxColor.fromRGB(ClientPrefs.editorBoxColors[0].red - 40, ClientPrefs.editorBoxColors[0].green - 40, ClientPrefs.editorBoxColors[0].blue - 40), FlxColor.fromRGB(ClientPrefs.editorBoxColors[1].red - 40, ClientPrefs.editorBoxColors[1].green - 40, ClientPrefs.editorBoxColors[1].blue - 40));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * ((_song.keys * 2) + 1), Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]), true, FlxColor.fromRGB(ClientPrefs.editorBoxColors[0].red - 40, ClientPrefs.editorBoxColors[0].green - 40, ClientPrefs.editorBoxColors[0].blue - 40), FlxColor.fromRGB(ClientPrefs.editorBoxColors[1].red - 40, ClientPrefs.editorBoxColors[1].green - 40, ClientPrefs.editorBoxColors[1].blue - 40));
		...
		gridLayer.add(line);
		var line = new FlxSprite().makeGraphic(5, FlxG.height, FlxColor.WHITE);
		line.setPosition((_song.keys == 7 ? GRID_SIZE * 8 : GRID_SIZE * 5) - 2.5, 0);
		line.scrollFactor.set(1, 0);
		gridLayer.add(line);

		if(UI_box != null && bpmTxt != null) setUIStuffPos();
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool, ?isPrevSection:Bool = false):Note
	{
		...
		var note:Note = new Note(daStrumTime, daNoteInfo % _song.keys, null, null, true);
	...
	function setupSusNote(note:Note, beats:Float):FlxSprite {
		var height:Int = Math.floor(FlxMath.remapToRange(note.sustainLength, 0, Conductor.stepCrotchet * 16, 0, GRID_SIZE * 16 * zoomList[curZoom]) + (GRID_SIZE * zoomList[curZoom]) - GRID_SIZE / 2);
	...
	function resetIdle(){
		lilOpp.color = ClientPrefs.editorBoxColors[0];
		lilBf.color = ClientPrefs.editorBoxColors[0];
		lilStage.color = ClientPrefs.editorBoxColors[1];
		lilBf.animation.play("idle");
		lilOpp.animation.play("idle");
	}
}
...
class ChartingInfoSubstate extends MusicBeatSubstate
{
	...
	public function new(){
		super();
		text =
		"Welcome to the Charting Info Hotkey / directions!
	...
	override public function update(elapsed:Float){
		...
		if(FlxG.keys.justPressed.ESCAPE){
			...
			FlxTween.tween(ChartingState.textBox, {x: 762.5, y: 433, alpha: 0.6}, 0.75, {ease: FlxEase.quartOut, startDelay: 0.25});
			FlxTween.color(ChartingState.textBox, 0.75, FlxColor.fromRGB(ClientPrefs.editorUIColor.red, ClientPrefs.editorUIColor.green, ClientPrefs.editorUIColor.blue), FlxColor.BLACK, {ease: FlxEase.quartOut, startDelay: 0.25});
			FlxTween.tween(ChartingState.textBox.scale, {x: 1, y: 1}, 0.75, {ease: FlxEase.quartOut, startDelay: 0.25, onComplete: function(shit:FlxTween){
				close();
			}});
		}
	}
...
class ChartingOptionsSubmenu extends MusicBeatSubstate
{
	...
	var menuItems:Array<String> = ['Resume', 'Play from beginning', 'Play from here', 'Set start time', 'Play from start time' /*, 'Botplay'*/, 'Exit to main menu']; //shamelessly stolen from andromeda im sorry
