...
class TitleState extends MusicBeatState
{
	...
	public var easterEggKeys:Array<String> = [
		'SHADOW', 'RIVER', /* LMAOOOOO 'SHUBS' */, 'BBPANZU'
	];
	public var allowedKeys:String = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
	...
	public function startIntro()
	{
		...
		if(hardcoded() && callOnScript('onStartIntro', []) != Globals.Function_Stop){
			...
			//trace(path, FileSystem.exists(path));
			if (!FileSystem.exists(path)){
				path = "assets/shared/images/titleEnter.png";
			}
			//trace(path, FileSystem.exists(path));
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			sickBeats++;
			setOnScript('sickBeats', sickBeats);
			if(hardcoded() && callOnScript('onBeatHit', [sickBeats]) != Globals.Function_Stop){
